...
void CTexture::CreateFromData(BYTE* bData, int iWidth, int iHeight, int iBPP, GLenum format, bool bGenerateMipMaps)
	...
	glBindTexture(m_textureTarget, m_uiTexture);

	if(format == GL_RGBA || format == GL_BGRA)
		glTexImage2D(m_textureTarget, 0, GL_RGBA, iWidth, iHeight, 0, format, GL_UNSIGNED_BYTE, bData);
	// We must handle this because of internal format parameter
	else if(format == GL_RGB || format == GL_BGR)
		glTexImage2D(m_textureTarget, 0, GL_RGB, iWidth, iHeight, 0, format, GL_UNSIGNED_BYTE, bData);
	else
		glTexImage2D(m_textureTarget, 0, format, iWidth, iHeight, 0, format, GL_UNSIGNED_BYTE, bData);
...
void CTexture::SetTarget(GLenum TextureTarget)
	{
