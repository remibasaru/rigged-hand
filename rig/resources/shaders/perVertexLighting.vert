...
uniform bool bUseBoneWeights;

// Layout of vertex attributes in VBO
layout (location = 0) in vec3 inPosition;
layout (location = 1) in vec2 inCoord;
layout (location = 2) in vec3 inNormal;
layout (location = 3) in float bone1Weight;
layout (location = 4) in float bone2Weight;
layout (location = 5) in float bone3Weight;
...
{	
	...
	if (bUseBoneWeights) {
		float sumWeights = bone1Weight + bone2Weight + bone3Weight;
		if (sumWeights > 0.1)
			p = (bone1Weight * matrices.bone1Xform + bone2Weight * matrices.bone2Xform + bone3Weight * matrices.bone3Xform) * vec4(inPosition, 1.0f);
	}		

	// Transform the vertex spatial position using 
	gl_Position = matrices.projMatrix * matrices.modelViewMatrix * p;
